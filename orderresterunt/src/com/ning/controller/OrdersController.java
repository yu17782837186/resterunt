...
public class OrdersController {
	...
	public String submitOrder(HttpSession session,String cids,String st){
		...
		//同时删除购物车相应的数据
		ic.delBatch(arrCids);
		return "findMyOrders.action";
	}
	
	//查询我的订单
	@RequestMapping("findMyOrders")
	public String findMyOrders(HttpSession session){
		User user = (User)session.getAttribute("userLogin");
		List<Ordersdetail> listOrders = io.findMyOrders(user.getUid());
		session.setAttribute("listOrders", listOrders);
		return "redirect:/ListOrders.jsp";
	}
	
	//发送
	@RequestMapping("updOrdersOstatus")
	public String updOrdersOstatus(String oid){
		io.updOrdersOstatus(oid);
		return "/OrdersdetailController/findOrdersdetail.action";
	}
	
	//签收
	@RequestMapping("updOrdersOstatus_2")
	public String updOrdersOstatus_2(String oid){
		io.updOrdersOstatus_2(oid);
		return "findMyOrders.action";
	}
